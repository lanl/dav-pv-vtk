...
class ELLLikeGraph:
    ...
    @classmethod
    def from_cube_skip_sparse(cls, dim, vals_np, threshold_index=100):
        """Create a sparse graph with edges only between the top `threshold_index` vertices when sorted using `vals_np`.
        """
        # print("sparse = ", sparse)
        dim_x = dim[0]
        ...
        y_idx = np.floor_divide(idx, dim[0], dtype=np.float64)
        x_idx = np.mod(idx, dim[0], dtype=np.float64)
        # Find which points are included
        all_even = np.logical_and(np.logical_and(x_idx % 2 == 0, y_idx % 2 == 0), z_idx % 2 == 0)
        all_odd = np.logical_and(np.logical_and(x_idx % 2 != 0, y_idx % 2 != 0), z_idx % 2 != 0)
        all_included = np.logical_or(all_even, all_odd)
        # print("num included: {0}/{1}".format(all_included.sum(), vals_np.size))
        test = vals_np[all_included]
        # print("after indexing: {0}/{1}".format(test.size, vals_np.size))
        threshold = np.partition(vals_np[all_included], -threshold_index)[-threshold_index]
        # print("threshold: {0} out of [{1}, {2}]".format(threshold, np.min(vals_np), np.max(vals_np)))
        # exit()
        values = np.append(vals_np, -1)
        # want to include: ~ 1, 1, 1 ~ 2, 2, 2
        # x-1, y-1, z+1 ~ 0, 0, 2 ~ 1, 1, 3
        # x-1, y+1, z+1 ~ 0, 2, 2 ~ 1, 3, 3
        # x-1, y-1, z-1 ~ 0, 0, 0 ~ 1, 1, 1
        # x-1, y+1, z-1 ~ 0, 2, 0 ~ 1, 3, 1
        # x+1, y-1, z+1 ~ 2, 0, 2 ~ 3, 1, 3
        # x+1, y+1, z+1 ~ 2, 2, 2 ~ 3, 3, 3
        # x+1, y-1, z-1 ~ 2, 0, 0 ~ 3, 1, 1
        # x+1, y+1, z-1 ~ 2, 2, 0 ~ 3, 1, 1
        for diff_x in [1, -1]:
            x_ = x_idx + diff_x
            x_[(x_ == -1) | (x_ == dim[0])] = np.NAN
            for diff_y in [1, -1]:
                y_ = y_idx + diff_y
                y_[(y_ == -1) | (y_ == dim[1])] = np.NAN
                # skip_even = np.logical_and(x_ % 2 == 0, y_ % 2 == 0)
                # skip_odd = np.logical_and(x_ % 2 != 0, y_ % 2 != 0)
                # x_[skip_even] = np.NAN
                # x_[skip_odd] = np.NAN
                # y_[skip_even] = np.NAN
                # y_[skip_odd] = np.NAN
                for diff_z in [1, -1]:
                    ...
                    graph.A[:, neighbors_idx] = indices
        graph.A[vals_np < threshold, :] = num_vertices  # no edges from low-value points
        # no edges from skipped points
        all_skipped = 1 - all_included
        graph.A[all_skipped, :] = num_vertices
        graph.num_edges = np.array(graph.A[:, 1:] < num_vertices).sum()
        print("num edges: ", graph.num_edges)
        return graph
    # End of from_cube_skip_sparse()
