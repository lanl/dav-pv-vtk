...
# from dds import graph_based_sampling
# from dds.graph import ELLLikeGraph
...
class DataManager(object):
    ...
    def __init__(self, fname, target_var_id):
        ...
        self.supported_extenstions = ['.vtp', '.vtk', '.vtu', '.vti', '.h5', '.hdf5']
        self.varName = ""
        self.data = []
        self.grad = []
        self.dataSpacing = []
    ...
    def update(self):        
        if self.f_extension in ['.h5', '.hdf5']:
            f = h5py.File(self.fname, 'r')
            print("read in hdf5 file")
            self.data = f["native_fields"]["baryon_density"][:]
            f.close()
            self.XDIM, self.YDIM, self.ZDIM = self.data.shape
            self.dataSpacing = [1, 1, 1]
            self.dataOrigin = [0, 0, 0]
            self.dataExtent = self.data.shape
            self.varName = "native_fields/baryon_density"
        else:
...
class SampleManager(object):
    ...
    def rand_sampling(self, data, sampling_ratio):
        ...
        return stencil   
    # End of rand_sampling()

    def lcc_sampling(self, data, dim, sampling_ratio):
        ...
        sparse = int(100 * (1.0 - sampling_ratio))
        graph = ELLLikeGraph.from_cube_sparse(dim, data, sparse)
        ...
        # print("Got connected component of size: {}".format(lcc_indices.size))
        prob_vals[lcc_indices] = 1
        # print("done sampling")

        # Sample contrast points
        if tot_samples > lcc_indices.size:
            threshold2 = np.percentile(data, sparse)
            indices = np.arange(0, samples)
            low_degree_seeds = indices[data < threshold2]
            # print("tot_samples: ", tot_samples, " lcc_size: ", lcc_indices.size)
            selected_low_degree_points = np.random.choice(low_degree_seeds, int(tot_samples - lcc_indices.size), False)
...
class ReconstructionManager(object):
    ...
    def reconstruction_1(self, block_size, sampled_data, sampled_lid, vhist_count, vhist_ble, vhist_delta):
    ...
    def reconstruction_2(self, block_size, sampled_data, sampled_lid, vhist_count, vhist_ble, vhist_delta):
    ...
    def griddata_nearest_reconstruction(self, block_size, sampled_data, sampled_lid, vhist_count, vhist_ble, vhist_delta,block_dims): 
    ...
    # griddata based interpolation
    def griddata_linear_reconstruction(self, block_size, sampled_data, sampled_lid, vhist_count, vhist_ble, vhist_delta,block_dims): 
    ...
    # natural neighbor based interpolation
    def reconstruction_7(self, block_size, sampled_data, sampled_lid, vhist_count, vhist_ble, vhist_delta,block_dims): 
        ...
        grid_ranges = [[0,block_dims[0],1],[0,block_dims[1],1],[0,block_dims[2],1]]
        
        #grid_z0 = griddata(feat_arr, sampled_data, cur_loc, method=cur_samp)
        grid_z0 = naturalneighbor.griddata(feat_arr, sampled_data, grid_ranges)
