...
def write_vti_output(full_recon_data, vti_folder, recontype):
    ...
    filename_base = "_".join(vti_folder.split("_")[2:])
    filename = "{0}/reconstructed_{1}_{2}.vti".format(vti_folder, filename_base, recontype)
    # filename = vti_folder + 'recons_data_' + str(sampling_rate) + '_' + sampling_method + '_' + str(recontype) + '.vti'
    print("Writing reconstructed data to: ", filename)
...
# End of write_vti_output()


def reeber2_calculation(full_recon_data, reeber_folder, recontype):
    print('Starting reeber halo computation.')
    t0 = time.time()

    if not os.path.exists(reeber_folder):
        print("Output path with halo output does not exist! Creating..")
        os.makedirs(reeber_folder)

    hdf5path = '/home/biswas/work/halo_pipeline/'
    hdf5_file_num = '5'
    inhdf5file = hdf5path + 'NVB_C009_l10n512_S12345T692_z' + hdf5_file_num + '.hdf5'
    # justfilename, justfile_extension = os.path.splitext(inhdf5file)
    cmd = 'cd ' + hdf5path + ';' + 'cp NVB_C009_l10n512_S12345T692_z' + hdf5_file_num + \
        '.hdf5 NVB_C009_l10n512_S12345T692_z' + hdf5_file_num + '_mod.hdf5'
    os.system(cmd)
    print('Done copying hdf5 file.')

    infile = hdf5path + 'NVB_C009_l10n512_S12345T692_z' + hdf5_file_num + '_mod.hdf5'
    f = h5py.File(infile, 'r+')
    data = f['native_fields/baryon_density']
    print(data.shape)
    ...
    np.allclose(f1['native_fields/baryon_density'][()], full_recon_data)

    # run this command from the command shelll
    # ~/work/halo_finder/hdf5_halo/reeber/build/examples/amr-connected-components/amr_connected_components_float -b 128 -n -w -f native_fields/baryon_density NVB_C009_l10n512_S12345T692_z5.hdf5 none none NVB_integral_128

    outputCSVname = reeber_folder + '/' + '_integral_128_' + str(sampling_rate) + '_' + sampling_method + '_' + str(
        recontype)
    ...
    cmd = '~/work/halo_finder/hdf5_halo/reeber/build/examples/amr-connected-components/amr_connected_components_float -b 128 -n -w -f native_fields/baryon_density ' + \
        infile + ' none none ' + outputCSVname
    os.system(cmd)
    print('Time taken %.2f secs' % (time.time() - t0))
# End of reeber2_calculation()
...
def power_spectrum_calculation(full_recon_data):
    ...
    # os.system(cmd)
    hdf5_file_num = '42'
    cmd = 'cp NVB_C009_l10n512_S12345T692_z' + hdf5_file_num + \
        '.hdf5 NVB_C009_l10n512_S12345T692_z' + hdf5_file_num + '_mod.hdf5'
    os.system(cmd)
    ...
    name_string = '_' + hdf5_file_num + '_recon_' + str(sampling_rate) + '_' + sampling_method + '_'
    cmd = '/Users/biswas/Work/gimlet2/apps/sim_stats/sim_stats.ex NVB_C009_l10n512_S12345T692_z' + \
        hdf5_file_num + '_mod.hdf5 power_spectrum/' + name_string
    os.system(cmd)
    ...
    print('Time taken %.2f secs' % (time.time() - t0))
# End of power_spectrum_calculation()
...
def run_reconstruction(nthreads, recontype):
    ...
    with pymp.Parallel(nthreads) as p:
        for bid in p.range(nob):
            ...
            if recontype == 1:
                recon_block = rm.reconstruction_1(XBLOCK * YBLOCK * ZBLOCK, list_sampled_data[bid],
                                                  list_sampled_lid[bid], array_void_hist[bid], array_ble[bid],
                                                  array_delta[bid])
            elif recontype == 2:
                recon_block = rm.reconstruction_2(XBLOCK * YBLOCK * ZBLOCK, list_sampled_data[bid],
                                                  list_sampled_lid[bid], array_void_hist[bid], array_ble[bid],
                                                  array_delta[bid])
            elif recontype == 3:
                recon_block = rm.reconstruction_3(XBLOCK * YBLOCK * ZBLOCK, list_sampled_data[bid],
                                                  list_sampled_lid[bid], array_void_hist[bid], array_ble[bid],
                                                  array_delta[bid], block_dims)
            elif recontype == 4:
                recon_block = rm.reconstruction_4(XBLOCK * YBLOCK * ZBLOCK, list_sampled_data[bid],
                                                  list_sampled_lid[bid], array_void_hist[bid], array_ble[bid],
                                                  array_delta[bid], block_dims)
            elif recontype == 5:
                recon_block = rm.griddata_nearest_reconstruction(XBLOCK * YBLOCK * ZBLOCK, list_sampled_data[bid],
                                                                 list_sampled_lid[bid], array_void_hist[bid],
                                                                 array_ble[bid], array_delta[bid], block_dims)
            elif recontype == 6:
                recon_block = rm.griddata_linear_reconstruction(XBLOCK * YBLOCK * ZBLOCK, list_sampled_data[bid],
                                                                list_sampled_lid[bid], array_void_hist[bid],
                                                                array_ble[bid], array_delta[bid], block_dims)
            else:
                recon_block = rm.reconstruction_1(XBLOCK * YBLOCK * ZBLOCK, list_sampled_data[bid],
