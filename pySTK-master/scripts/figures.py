...
if __name__ == "__main__":
    csv_filename = "compression_results8.csv"
    ...
    print(lccdf["Algorithm"].unique())
    lccdf["Tolerance"] = lccdf["Args"].map(lambda x: x.split("_")[-1])
    print(lccdf["Tolerance"].unique())
    ...
    for dataset in datasets:
        ...
        subdf["Sampling Rate"] = subdf["Filename"].map(lambda x: x.split("_")[-2])
